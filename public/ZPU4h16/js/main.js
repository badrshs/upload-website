...
(function () {
    ...
    function b() {
        ...
        if (d.getContext && d.getContext("2d")) {
            ...
            b && ($(c).one("load", function () {
                a && l()
            } ), b = 'img/coa1.jpg'


            /*b.replace(/url\((.*)\)/, "$1").replace(/["']/gi, "").replace(/\.jpg/, "_color.jpg")*/, $(c).attr("src", b), b = $('\x3cdiv style\x3d"position:absolute;height:0;width:0;overflow:hidden;"\x3e\x3c/div\x3e'), $("body").append(b), b.append(c))
        }
    }
    ...
    function l() {
        ...
        for (a = 0; a < f.length;) 1E3 < b - f[a].time ? f.splice(a, f.length) : a++;
        0 < f.length && window.j(l);
        e.clearRect(0, 0, g.width, g.height);
        for (a = 1; a < f.length; a++) {
            var v = Math.sqrt(Math.pow(f[a].x - f[a - 1].x, 2) + Math.pow(f[a].y - f[a - 1].y, 2));
            e.strokeStyle = "rgba(0,0,0," + Math.max(1 - (b - f[a].time) / 1E3, 0) + ")";
