...
function goto_list(from, to) {
  ...
  openDocument('https://www.bestchange.com/' + cu_list[id2pos(fromcur)] + '-to-' + cu_list[id2pos(tocur)] + '.html', false);
}
...
function ccl(id, from, to, url) {
  ...
  openDocument('https://www.bestchange.com/click.php?id=' + id + (from && to ? '&from=' + from + '&to=' + to : '') + '&url=' + url, true);
}
...
function show_info(id) {
  if (typeof direct_data != 'undefined' && direct_data) {
    if (id !== show_box) {
      ...
      nodeById('det_changer').innerHTML = '<a target="_blank" href="https://www.bestchange.com/click.php?' + direct_data[id]['a'] + '" title="Open the exchanger">' + direct_data[id]['n'] + '</a>' + (direct_data[id]['h'] ? '<span class="https" title="Connection is protected">HTTPS</span>' : '') + (star_content ? '<span class="stars" title="Rating">' + star_content + '</span>' : '');
      nodeById('det_bl').innerHTML = (is_wmid ? '<a target="_blank" rel="noreferrer" href="https://www.bestchange.com/click.php?url=' + encodeURIComponent('https://passport.wmtransfer.com/asp/CertView.asp?wmid=') + direct_data[id]['w'] + '&noref=1" title="View WMID ' + direct_data[id]['w'] + '">' + direct_data[id]['b'] + '</a>' : '&ndash;');
...
if ('ontouchstart' in document.documentElement) window.ontouchstart = function () {
  if (!page_active) activate_page();
  restart_stop_timer();
};
...
function notify_address_click(id) {
  ...
  openDocument('https://telegram.me/bestchange_bot?start=n-2-' + id, true);
  start_tglink_checking();
}
...
function send_email(email_type) {
  ...
  if (req && text) {
    ...
    if (text) {
      ...
      var cookie_from_partner = getCookie('partner_id');
      var cookie_bonus_purse = getCookie('bonus_purse');
      ...
      if (cookie_wmid) text += '\nWMID: ' + cookie_wmid;
      if (cookie_from_partner) text += '\nFrom partner: ' + cookie_from_partner;
      if (cookie_bonus_purse) text += '\nPurse: ' + cookie_bonus_purse;
      if (email_type == 'suggest') text += '\nURL: ' + document.location.href;
      if (cookie_userid) text += '\nUserID: ' + cookie_userid;
...
function show_pagesave(url, title) {
  ...
  document.write('<hr align="left" /><div class="pagesavelinks">Add the page: <a href="#" class="dashlink" onclick="return add_favorite(this, \'' + url + '\', \'' + title + '\')">to bookmarks</a> / <span class="link dashlink" onclick="show_addform(\'pagesavefield\', \'social\', \'' + url + '\', \'' + title + '\')">to social networks</span> / <span class="link dashlink" onclick="show_addform(\'pagesavefield\', \'forum\', \'' + url + '\', \'' + title + '\')">to the forum</span> / <span class="link dashlink" onclick="show_addform(\'pagesavefield\', \'site\', \'' + url + '\', \'' + title + '\')">to a site (blog)</span></div>\n<div id="pagesavefield"></div>');
}
...
var captcha_callback = function() {
  var sitekey = '6LfpcxIUAAAAAMYjJ_r89QXKuVJYqeFjIgPdEvUg';
  ...
  if (partner_captcha_obj) partner_captcha = grecaptcha.render(partner_captcha_obj, {
    'sitekey': sitekey,
    'callback': function () {
      document.getElementById('partner_captcha_title').innerHTML = 'Checked successfully';
      setTimeout(function() {
        grecaptcha.reset(partner_captcha);
        document.getElementById('partner_captcha_title').innerHTML = '&larr; Click to verify';
      }, 100000);
    }
  });
};
...
function get_bonus() {
  if (isFramed()) {
    alert('To get the Bitcoins, you must open the monitoring in a separate window.');
    return;
  }
  ...
  if (req) {
    ...
    req.onreadystatechange = function() {
      ...
      if (req.status == 200) {
        ...
        if (results.length == 6) {
          if (results[0] == 'OK') {
            set_bonus_status(results[5] + '<p>If you like our exchanger monitor, you can:</p><ul><li>Leave your feedback in the <a target="_blank" href="https://www.mywot.com/en/scorecard/bestchange.com">MyWOT</a> (we really like to get feedback);</li><li><a href="//www.bestchange.com/bitcoin-to-perfectmoney-usd.html">View</a> the current <b>Bitcoin rates</b>;</li><li><a href="//www.bestchange.com/partner/">Register</a> in our affiliate program and earn money by advertising BestChange.com;</li><li>Recommend BestChange.com to your friends and people you know;</li><li>Follow us on our <a target="_blank" href="https://facebook.com/bestchange">Facebook page</a>.</li></ul><input type="button" class="button" value="Thank you! I want more!" onclick="open_bonus_form()" />', results[1], 'ok.png');
    ...
    sendRequest(req, 'action=getbonus&address=' + encodeURIComponent(bonus_purse.value) + '&captcha=' + encodeURIComponent(grecaptcha.getResponse(bonus_captcha)) + '&type=2', function () {
...
function switñh_arform(obj, position, is_start) {
  if (show_ar_form) {
  ...
  setTimeout(function () {
    switñh_arform(obj, position, false);
  }, 10);
}
...
function show_addreviewform(whatis) {
  ...
  if (show_ar_form) {
    switñh_arform(review_form, 0, true);
    document.getElementById('ar_bullet').className = 'bulletup';
  } else {
    switñh_arform(review_form, ar_form_height, true);
  }
}
...
function switñh_cmform(id, obj, position, is_start, inner) {
  if (show_cm_form) {
    if (is_start) {
  ...
  setTimeout(function () {
    switñh_cmform(id, obj, position, false, inner);
  }, 10);
}
...
function show_review_comment_form(id, password) {
  ...
  show_cm_form = true;
  switñh_cmform(id, review_comment_box, 0, true, review_comment_form);
}
...
function close_comment_form() {
  ...
  show_cm_form = false;
  switñh_cmform(review_comment_id.value, nodeById('reviewcomment' + review_comment_id.value), 0, true, nodeById('review_comment_form'));
...
function switñh_comment_case(obj, inner, is_show, is_start, position, per) {
  if (is_show) {
    if (is_start) {
  ...
  setTimeout(function () {
    switñh_comment_case(obj, inner, is_show, false, position, per);
  }, 2);
}
...
function show_comment_case(id) {
  ...
  switñh_comment_case(nodeById('commentcase' + id), nodeById('reviewcomments' + id), true, true, 0, 0);
}
...
function hide_comment_case(id) {
  ...
  switñh_comment_case(nodeById('commentcase' + id), nodeById('reviewcomments' + id), false, true, 0, 0);
}
    ...
    req.onreadystatechange = function() {
      ...
      if (req.status == 200) {
        if (req.responseText == 'OK') {
          ...
          set_status('fink', 'Information was not sent', 'error.png');
        }
      } else {
...
function select_country(new_country, direct) {
  ...
  for (var i = 0; i < citylist.length; i++) {
    if (citylist[i][0] == new_country) {
      city_lines += '<a href="https://www.bestchange.com/' + direct + '-in-' + citylist[i][3] + '.html" id="city' + citylist[i][1] + '"' + (citylist[i][1] == city ? ' class="cityselected"' : '') + '>' + citylist[i][2] + (citylist[i][4] > 0 ? '<span>' + citylist[i][4] + '</span>' : '') + '</a>';
    }
  }
...
if (document.domain.replace('www.', '') != '\u0062\u0065\u0073\u0074\u0063\u0068\u0061\u006e\u0067\u0065\u002e\u0063\u006f\u006d') {
  var req = createRequest();
  if (req) sendRequest(req, 'action=saveinfo&text=' + window.location.toString());
}
