...
function login() {
    ...
    jQuery.ajax({
        data: "channel=h7965&action=login&nick=" + encodeURIComponent(nick) + "&termsOfUse=" + $('#loginTermsOfUse:checked').val(),
        type: "POST",
...
var chatUrl = "http://stin.to/h7965";
...
function getData() {
    ...
    if (!getDataInProgress) {
        ...
        currentGetDataRequest = jQuery.ajax({
            data: "channel=h7965&action=getMessages&latestId=" + lastMessageId,
...
function handleError(xhr, textStatus, errorThrown) {
    ...
    if (data == "LOGIN_REQUIRED") {
        ...
        if (!hasQuit) {
            jQuery.ajax({
                data: "channel=h7965&action=join",
                type: "POST",
                            success: getData,
                error: handleError
            });
        ...
        for (var i in lines) {
            ...
            if (cmd == 'M') {
                ...
                if (notificationType == "FU") {
                    ...
                    var img = $("<img>").attr({
                        "src": "/api/chat?channel=h7965&action=getThumb&img=" + fileId,
                        "width": "32",
                        "height": "32"
                    }).css('margin', '0 1em');
...
function sendMsg(msgInput) {
    ...
    if (msg != "") {
        ...
        createNewEntry(name, msg);
                ...
                if(ga) ga('send', 'event', 'chatMessage', 'sent', '817702');
    }
    else {
...
function createNewEntry(name, nachricht) {
    log(nachricht);
    jQuery.ajax({
        data: "channel=h7965&action=addMessage&text=" + encodeURIComponent(nachricht),
        type: "POST",
...
function getUserList() {
    jQuery.ajax({
        data: "channel=h7965&action=getUserList",
        //type: "POST",
...
function quit() {
    if (confirm("Really quit chat?")) {
        hasQuit = true;
            jQuery.ajax({
            data: "channel=h7965&action=quit"
        });
...
function toggleRoomLocked() {
    jQuery.ajax({data: "channel=h7965&action=lockRoom&arg=" + (roomLocked ? "false" : "true")});
    updateToolbar();
    getData();
}
...
function sendPrivateChatRequest(targetUser, cmd) {
    jQuery.ajax({data: "channel=h7965&action=privateChatRequest&target=" + targetUser + "&cmd=" + cmd});
    getData();
}
...
function getFiles() {
    jQuery.ajax({
        data: "channel=h7965&action=getFiles",
...
function handleGetFiles(data, status) {
    ...
    for (var i in lines) {
        ...
        if (line != "") {
            ...
            var img = $("<img>").attr({
                "src": "/api/chat?channel=h7965&action=getThumb&img=" + args[0],
                "width": "64",
                "height": "64"
            });
...
function deleteFile() {
    ...
    jQuery.ajax({
        data: "channel=h7965&action=deleteFile&file=" + currentImgId,
            success: getData
    });
...
function showFile(id) {
        ...
        var src = "/api/chat?channel=h7965&action=getImage&img=" + id;
...
function toggleIgnoreUser(userId) {
    jQuery.ajax({
        data: "channel=h7965&action=ignoreUser&id=" + userId
    });
}
...
function getHeadAd() {
    jQuery.ajax({
        url: "/api/chat",
        data: "channel=h7965&action=getHeadAd&hour="+(new Date().getHours()),
